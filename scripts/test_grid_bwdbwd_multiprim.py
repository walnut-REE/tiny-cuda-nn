...
class SDF(nn.Module):
	def __init__(self, hash=True, n_levels=12, n_primitives=8, log2_hashmap_size=15, base_resolution=16, smoothstep=False) -> None:
		super().__init__()
		...
		print('*** Encoding params: ', len(self.encoder.params))
		# print(self.encoder.params)
	...
	def forward(self, x):
		...
		# print('*** encoder output: ', encoded.shape, encoded.max(), encoded.min())
		sdf = self.decoder(encoded)
		# print('*** decoder output: ', sdf.shape, sdf.max(), sdf.min())
		return sdf
...
if __name__ == '__main__':
	...
	def test_():
		...
		model = SDF(True, n_levels=16, n_primitives=N_PRIMITIVES, log2_hashmap_size=15, base_resolution=4, smoothstep=False).to(device)
		x = (torch.tensor([[0.3, 0.4, 0.5]]*N_PRIMITIVES) + torch.arange(N_PRIMITIVES)[..., None]).float().to(device).requires_grad_(True)
	...
	def test_train():
		...
		model = SDF(True, 4, n_primitives=N_PRIMITIVES, base_resolution=12).to(device)
		...
		with tqdm(range(10000)) as pbar:
			for _ in pbar:
				...
				x = x + torch.randint(0, N_PRIMITIVES, (51200, 1), dtype=torch.float, device=device)
	...
	def grad_check():
		...
		# model = SDF(True, n_levels=4, n_primitives=8, log2_hashmap_size=19, base_resolution=4, smoothstep=True).to(device)
		model = SDF(True, n_levels=1, n_primitives=N_PRIMITIVES, log2_hashmap_size=15, base_resolution=8, smoothstep=False).to(device)
		...
		autograd.gradcheck(
			...
			(torch.tensor([[0.17, 0.55, 0.79]]*N_PRIMITIVES) + torch.arange(N_PRIMITIVES)[..., None]).float().to(device).requires_grad_(True),
			eps=1.0e-3)
		print("[PASS] gradcheck apply_on_x.")
		...
		print("[PASS] gradcheck apply_on_x pass 2.")
		...
		def backward_apply_on_params(params):
			x = (torch.tensor([[0.17, 0.55, 0.79]]*N_PRIMITIVES) + torch.arange(N_PRIMITIVES)[..., None]).float().to(device).requires_grad_(True)
			dL_dy = torch.ones([*x.shape[:-1], model.encoder.n_output_dims], dtype=dtype, device=device)
		...
		print("[PASS] gradcheck backward_apply_on_params.")
		...
		print("[PASS] gradcheck backward_apply_on_dLdy.")
